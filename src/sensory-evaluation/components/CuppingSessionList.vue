...
import { saveCuppingSession, getCuppingSessions } from '../services/cuppingService'
...
const handleNewCuppingCreated = async (newSession) => {
  try {
    ...
    const saved = await saveCuppingSession(newSessionData);
    cuppingSessions.value.push(saved);
    router.push(`/cata/${Number(saved.id)}`);
  } catch (error) {
...
onMounted(async () => {
  ...
  try {
    cuppingSessions.value = await getCuppingSessions();
  } catch (error) {
    console.error('Error al obtener las sesiones:', error);
    alert('No se pudieron cargar las sesiones de cata.');
  }
})
